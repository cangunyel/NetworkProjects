...
class FourSectionsGUI:
    def __init__(self, master):
        ...
        self.result_text = scrolledtext.ScrolledText(self.frame3, wrap=tk.WORD, width=30)
        ...
        self.tk_image1 = None
        self.tk_image2 = None
        self.image_path1 = "rdt3/rdt3reciver_wait0.png"
        self.image_path2 = "rdt3/rdt3sender_waitcall0.png"
    ...
    def check_input(self):
        ...
        if user_input == "corrupt":
            ...
            self.master.after(3000, lambda: self.text_part1.insert(tk.END, "rdt_rcv(rcvpkt)&&(corrupt(rcvpkt)||has_seq(rcvpkt)\n\n"+"sndpkt=make_pkt(ACK,chksum)\nudt_send(sndpkt)") )
            self.master.after(3500, lambda: self.text_part2.insert(tk.END, "rdt_rcv(rcvpkt)&&(corrupt(rcvpkt)||isACK(rcvpkt))")  )       

        if user_input == "lost":
            self.clear_text_parts()
            self.master.after(4500, lambda: self.text_part2.insert(tk.END, "timeout\n\n"+"udt_send(sndpkt)\nstart_timer") )
    ...
    def update_results(self):
        ...
        if received_message:
            ...
            if re.match(r"ACK \d timed out\.", received_message):
                if(random.choice([0, 1])):
                    self.result_text.insert(tk.END, "                                                       X<---------------" + '\n')
                else:
                    self.result_text.insert(tk.END, "                                       --------------->X                " + '\n')
            if re.match(r"ACK \d sent\.", received_message) or re.match(r"NAK \d sent\.", received_message):
                self.result_text.insert(tk.END, "                                       <--------------------------------" + '\n')
            # sender
            if re.match(r"Packet 1 sent\.", received_message):
                self.switch_photos(new_path2="rdt3/rdt3sender_waitACK1.png")
                self.clear_text_parts()
                self.text_part2.insert(tk.END, "rdt_send(data)\n\n"+"sndpkt = make_pkt(1, data, checksum)\nudt_send(sndpkt)\nstart_timer")
            elif re.match(r"Packet 0 sent\.", received_message):
                self.switch_photos(new_path2="rdt3/rdt3sender_waitACK0.png")
                self.clear_text_parts()
                self.text_part2.insert(tk.END, "rdt_send(data)\n\n"+"sndpkt = make_pkt(0, data, checksum)\nudt_send(sndpkt)\nstart_timer")
            elif re.match(r"ACK 1 sent\.", received_message):
                self.switch_photos(new_path2="rdt3/rdt3sender_waitcall0.png")
                self.clear_text_parts()
                self.text_part2.insert(tk.END,"rdt_rcv(rcvpkt)&& notcorrupt(rcvpkt)&& isACK(rcvpkt, 1)\n\n"+"stop_timer")
            elif re.match(r"ACK 0 sent\.", received_message):
                self.switch_photos(new_path2="rdt3/rdt3sender_waitcall1.png")
                self.clear_text_parts()
                self.text_part2.insert(tk.END,"rdt_rcv(rcvpkt)&& notcorrupt(rcvpkt)&& isACK(rcvpkt, 0)\n\n"+"stop_timer")
            # receiver
            if re.match(r"Packet 0 received\.", received_message):
                self.switch_photos(new_path1="rdt3/rdt3reciver_wait1.png")
                self.clear_text_parts()
                self.text_part1.insert(tk.END,"rdt_rcv(rcvpkt) && notcorrupt(rcvpkt)&& has_seq0(rcvpkt)\n\n"+"extract(rcvpkt,data)\ndeliver_data(data)\nsndpkt = make_pkt(ACK, 0, chksum)\nudt_send(sndpkt)")
                
            elif re.match(r"Packet 1 received\.", received_message):
                self.switch_photos(new_path1="rdt3/rdt3reciver_wait0.png")
                self.clear_text_parts()
                self.text_part1.insert(tk.END,"rdt_rcv(rcvpkt) && notcorrupt(rcvpkt)&& has_seq1(rcvpkt)\n\n"+"extract(rcvpkt,data)\ndeliver_data(data)\nsndpkt = make_pkt(ACK, 1, chksum)\nudt_send(sndpkt)")
            # figure
